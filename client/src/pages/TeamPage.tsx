...
export default function TeamPage() {
  ...
  const fetchTeams = async () => {
    try {
      ...
      toast.error("Failed to load teams index")
      setLoading(false)
    }
  }
  ...
  const createTeam = async () => {
    ...
    try {
      ...
      toast.success("Team space initialized")
    } catch (error) {
  ...
  const addMember = async (teamId: string) => {
    if (!memberEmail.trim()) {
      toast.error("Email verification address required")
      return
    }
    ...
    try {
      ...
      toast.success("New member added to team array")
    } catch (error) {
  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center min-h-screen bg-[#050505] font-mono text-xs text-[#00e87a]">
        <span className="animate-pulse mr-2">▋</span> Loading teams registry...
      </div>
    )
  }
    ...
    >
        ...
        <div className="max-w-6xl mx-auto px-6 h-14 flex items-center justify-between">
          <div className="flex items-center gap-2.5">
            ...
            <span className="font-mono text-sm font-bold tracking-tight text-white">
              collab<span className="text-[#00e87a]">code</span>
            </span>
        ...
        {teams.length === 0 ? (
          <div className="flex items-center justify-center min-h-[45vh] animate-in fade-in duration-300">
            <div className="bg-[#0a0a0a] border border-[#141414] rounded-xl p-10 max-w-md w-full text-center shadow-[0_20px_40px_-15px_rgba(0,0,0,0.8)]">
              ...
              <div className="font-mono text-sm font-semibold text-white mb-2">No Clusters Connected</div>
              <p className="text-xs text-[#666] leading-relaxed mb-6">
                You haven't established or been added to any collaborator workspace layers yet. Initialize a new team target to begin coding together.
              </p>
              <button
                onClick={() => setShowCreateModal(true)}
              ...
              >
                <Plus className="w-3.5 h-3.5" strokeWidth={2.5} /> Initialize Workspace Team
              </button>
            </div>
          </div>
        ) : (
                ...
                <div className="p-5 flex flex-col flex-1 justify-between">
                  <div>
                    ...
                    <p className="text-xs text-[#737373] mt-1.5 line-clamp-2 leading-relaxed">
                      {team.description || "No description provided for this specific instance folder."}
                    </p>
                  </div>
                  ...
                  <div className="border-t border-[#121212] pt-3.5 flex items-center justify-between mt-auto">
                    <div className="flex items-center gap-4 font-mono text-[11px] text-[#555]">
                      <div className="flex items-center gap-1">
                        ...
                        <span><strong className="text-[#888] font-medium">{team.members.length}</strong> user paths</span>
                      </div>
                      <div className="flex items-center gap-1">
                        <Layers className="w-3 h-3 text-[#333]" />
                        <span><strong className="text-[#888] font-medium">{team.projects.length}</strong> components</span>
                      </div>
                    </div>
          ...
          <div className="bg-[#0a0a0a] border border-[#1c1c1c] rounded-xl max-w-sm w-full overflow-hidden shadow-[0_30px_60px_-15px_rgba(0,0,0,0.9)] animate-in slide-in-from-bottom-3 duration-200">
            ...
            <div className="bg-[#060606] border-b border-[#141414] h-11 px-4 flex items-center justify-between">
              <div className="flex items-center gap-2">
                ...
                <span className="font-mono text-[11px] text-[#555] font-medium">initialize_team.sh</span>
              </div>
            ...
            {/* Inputs Body form */}
            <div className="p-6">
              <h2 className="font-mono text-sm font-bold text-white mb-1">Create Team Node</h2>
              <p className="text-xs text-[#666] leading-relaxed mb-5">Provision a shared directory workspace for collaborative projects.</p>
              
              <label className="font-mono text-[10px] text-[#555] block mb-1.5 tracking-wider uppercase">Team Space Name</label>
              <input
                type="text"
                value={teamName}
                onChange={(e) => setTeamName(e.target.value)}
                placeholder="e.g., frontend-core-mesh"
              ...
              <label className="font-mono text-[10px] text-[#555] block mb-1.5 tracking-wider uppercase">Manifest Descriptor</label>
              <textarea
                value={teamDesc}
                onChange={(e) => setTeamDesc(e.target.value)}
                placeholder="Briefly define core collaborative variables..."
        ...
        <div className="fixed inset-0 z-50 bg-black/85 backdrop-blur-md flex items-center justify-center p-4 animate-in fade-in duration-200">
          <div className="bg-[#0a0a0a] border border-[#1c1c1c] rounded-xl max-w-sm w-full overflow-hidden shadow-[0_30px_60px_-15px_rgba(0,0,0,0.9)] animate-in slide-in-from-bottom-3 duration-200">
            ...
            <div className="bg-[#060606] border-b border-[#141414] h-11 px-4 flex items-center justify-between">
              <div className="flex items-center gap-2">
                <div className="w-2.5 h-2.5 rounded-full bg-[#00e87a] shadow-[0_0_8px_#00e87a]" />
                <span className="font-mono text-[11px] text-[#555] font-medium">invite_pipeline.sh</span>
              </div>
            ...
            {/* Form body container */}
            <div className="p-6">
              <h2 className="font-mono text-sm font-bold text-white mb-1">Link Target User</h2>
              <p className="text-xs text-[#666] leading-relaxed mb-5">
                Inject access token paths into <span className="text-[#00e87a] font-mono">{selectedTeam.name}</span> index.
              </p>
              
              <label className="font-mono text-[10px] text-[#555] block mb-1.5 tracking-wider uppercase">User Mail Address</label>
              <input
                type="email"
                value={memberEmail}
                onChange={(e) => setMemberEmail(e.target.value)}
                placeholder="collaborator@network.mesh"
      ...
      <div className="fixed bottom-0 left-0 right-0 z-40 bg-[#030303] border-t border-[#111] h-7 flex items-center px-5 gap-6 font-mono text-[10px] text-[#444] font-medium select-none">
        <div className="flex items-center gap-1.5">
          ...
          <span>MESH_SYNC: SYNCED</span>
        </div>
        <div className="ml-auto text-[#333]">
          [ {teams.length} ] INSTANCES INDEXED
        </div>
      </div>
    </div>
  )
